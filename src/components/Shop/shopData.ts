// import { Product } from "@/types/product";
// const shopData: Product[] = [
//   {
//     title: "Havit HV-G69 USB Gamepad",
//     reviews: 15,
//     price: 59.0,
//     discountedPrice: 29.0,
//     id: 1,
//     imgs: {
//       thumbnails: [
//         "/images/products/product-1-sm-1.png",
//         "/images/products/product-1-sm-2.png",
//       ],
//       previews: [
//         "/images/products/product-1-bg-1.png",
//         "/images/products/product-1-bg-2.png",
//       ],
//     },
//   },
//   {
//     title: "iPhone 14 Plus , 6/128GB",
//     reviews: 5,
//     price: 899.0,
//     discountedPrice: 99.0,
//     id: 2,
//     imgs: {
//       thumbnails: [
//         "/images/products/product-2-sm-1.png",
//         "/images/products/product-2-sm-2.png",
//       ],
//       previews: [
//         "/images/products/product-2-bg-1.png",
//         "/images/products/product-2-bg-2.png",
//       ],
//     },
//   },
//   {
//     title: "Apple iMac M1 24-inch 2021",
//     reviews: 5,
//     price: 59.0,
//     discountedPrice: 29.0,
//     id: 3,
//     imgs: {
//       thumbnails: [
//         "/images/products/product-3-sm-1.png",
//         "/images/products/product-3-sm-2.png",
//       ],
//       previews: [
//         "/images/products/product-3-bg-1.png",
//         "/images/products/product-3-bg-2.png",
//       ],
//     },
//   },
//   {
//     title: "MacBook Air M1 chip, 8/256GB",
//     reviews: 6,
//     price: 59.0,
//     discountedPrice: 29.0,
//     id: 4,
//     imgs: {
//       thumbnails: [
//         "/images/products/product-4-sm-1.png",
//         "/images/products/product-4-sm-2.png",
//       ],
//       previews: [
//         "/images/products/product-4-bg-1.png",
//         "/images/products/product-4-bg-2.png",
//       ],
//     },
//   },
//   {
//     title: "Apple Watch Ultra",
//     reviews: 3,
//     price: 99.0,
//     discountedPrice: 29.0,
//     id: 5,
//     imgs: {
//       thumbnails: [
//         "/images/products/product-5-sm-1.png",
//         "/images/products/product-5-sm-2.png",
//       ],
//       previews: [
//         "/images/products/product-5-bg-1.png",
//         "/images/products/product-5-bg-2.png",
//       ],
//     },
//   },
//   {
//     title: "Logitech MX Master 3 Mouse",
//     reviews: 15,
//     price: 59.0,
//     discountedPrice: 29.0,
//     id: 6,
//     imgs: {
//       thumbnails: [
//         "/images/products/product-6-sm-1.png",
//         "/images/products/product-6-sm-2.png",
//       ],
//       previews: [
//         "/images/products/product-6-bg-1.png",
//         "/images/products/product-6-bg-2.png",
//       ],
//     },
//   },
//   {
//     title: "Apple iPad Air 5th Gen - 64GB",
//     reviews: 15,
//     price: 59.0,
//     discountedPrice: 29.0,
//     id: 7,
//     imgs: {
//       thumbnails: [
//         "/images/products/product-7-sm-1.png",
//         "/images/products/product-7-sm-2.png",
//       ],
//       previews: [
//         "/images/products/product-7-bg-1.png",
//         "/images/products/product-7-bg-2.png",
//       ],
//     },
//   },
//   {
//     title: "Asus RT Dual Band Router",
//     reviews: 15,
//     price: 59.0,
//     discountedPrice: 29.0,
//     id: 8,
//     imgs: {
//       thumbnails: [
//         "/images/products/product-8-sm-1.png",
//         "/images/products/product-8-sm-2.png",
//       ],
//       previews: [
//         "/images/products/product-8-bg-1.png",
//         "/images/products/product-8-bg-2.png",
//       ],
//     },
//   },
// ];

// export default shopData;
import { Product } from "@/types/product";

const shopData: Product[] = [
  {
    title: "Canva Pro - 1 Year Subscription",
    reviews: 120,
    price: 199.0,
    discountedPrice: 49.0,
    id: 1,
    imgs: {
      thumbnails: [
        "/images/products/canva-sm-1.png",
        "/images/products/canva-sm-2.png",
      ],
      previews: [
        "/images/products/canva-bg-1.png",
        "/images/products/canva-bg-2.png",
      ],
    },
  },
  {
    title: "Envato Elements - 1 Month Access",
    reviews: 95,
    price: 79.0,
    discountedPrice: 19.0,
    id: 2,
    imgs: {
      thumbnails: [
        "/images/products/envato-sm-1.png",
        "/images/products/envato-sm-2.png",
      ],
      previews: [
        "/images/products/envato-bg-1.png",
        "/images/products/envato-bg-2.png",
      ],
    },
  },
  {
    title: "Grammarly Premium - 6 Months",
    reviews: 88,
    price: 149.0,
    discountedPrice: 39.0,
    id: 3,
    imgs: {
      thumbnails: [
        "/images/products/grammarly-sm-1.png",
        "/images/products/grammarly-sm-2.png",
      ],
      previews: [
        "/images/products/grammarly-bg-1.png",
        "/images/products/grammarly-bg-2.png",
      ],
    },
  },
  {
    title: "Microsoft Office 365 - Lifetime License",
    reviews: 210,
    price: 299.0,
    discountedPrice: 59.0,
    id: 4,
    imgs: {
      thumbnails: [
        "/images/products/office-sm-1.png",
        "/images/products/office-sm-2.png",
      ],
      previews: [
        "/images/products/office-bg-1.png",
        "/images/products/office-bg-2.png",
      ],
    },
  },
  {
    title: "ChatGPT Plus - 1 Month Subscription",
    reviews: 150,
    price: 39.0,
    discountedPrice: 19.0,
    id: 5,
    imgs: {
      thumbnails: [
        "/images/products/chatgpt-sm-1.png",
        "/images/products/chatgpt-sm-2.png",
      ],
      previews: [
        "/images/products/chatgpt-bg-1.png",
        "/images/products/chatgpt-bg-2.png",
      ],
    },
  },
];

export default shopData;
